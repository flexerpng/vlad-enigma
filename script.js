...
function initializeMiners() {
    ...
    CONFIG.MINERS.forEach(miner => {
        ...
        minerCard.innerHTML = `
            ...
            <button id="buy-${miner.id}" class="buy-button">Купить (${miner.basePrice} USDT)</button>
            <button id="upgrade-${miner.id}" class="upgrade-button">Улучшить</button>
        `;
        minersGrid.appendChild(minerCard);

        // Добавляем обработчики событий для кнопок
        document.getElementById(`buy-${miner.id}`).addEventListener('click', () => buyMiner(miner.id));
        document.getElementById(`upgrade-${miner.id}`).addEventListener('click', () => upgradeMiner(miner.id));
    });
}
